...
items=[]    # Seperate container for indivisual items
bundles=[]  # Seperate container for product Bundles
```
...
cart = [] # For storing objects of class CartItem
cart.add(item)
cart.add(bundle)
...
shoppingCart.add(new Item("Shampoo", 9.99))
shoppingCart.add(techBundle)
